                      ...
                      <a-row type="flex" justify="space-between" align="bottom">
                        ...
                        <a-col class="applyTrue" :span="8">
                          <viewer :images="[applePeoplePh.V11]">
                            <img style="width: 40px;height:30px;" :src="applePeoplePh.V11? applePeoplePh.V11:''" alt="申请人签字" />
                          </viewer>
                        </a-col>
                        <a-col class="applyTrue" :span="4">
                          <div>
  ...
  left: 84%; top: 0;"
          ...
          >
            ...
            <template slot="action" slot-scope="text, record">
              <a slot="action" v-permission="{active:'zonsele'}" @click="fint(record)">查看&nbsp;&nbsp;</a>
            </template>
          </a-table>
...
const columns = [
  ...
  { title: '养殖场名称', width: 250, align: 'center', dataIndex: 'FyzName', key: 'FyzName' },
  { title: '地址',
    width: 250,
    align: 'center',
    dataIndex: 'Fdz',
...
export default {
  ...
  methods: {
    ...
    // 基层动监站已审核查询
     checkapply () {
       this.$axios
        .get(
          http +
            '/After/Auditcomplete?Fyzname=' + this.Fyzname + '&FSunitId=' + this.FSunitId + '&offset=' +
            this.offset +
            '&limit=' +
            this.limit +
            '&FScTime=' +
    ...
    // 审核通过
     auditOK () {
       if (!this.applyInfoFrom.getFieldValue('shenheren').replace(/^\s*|\s*$/g, '')) {
         this.$notification.error({
           message: '请输入审核人'
         })
       } else {
