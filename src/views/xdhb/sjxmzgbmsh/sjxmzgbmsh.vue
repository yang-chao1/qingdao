                      ...
                      <a-row type="flex" justify="space-between" align="bottom">
                        ...
                        <a-col class="applyTrue" :span="8">
                          ...
                          <!-- <a-form-item :wrapperCol="{ lg: { span: 16 }, sm: { span: 16 } }">
                            <a-input v-decorator="['shenhedanwei']" disabled style="margin-left:10px;" />
                          </a-form-item> -->
                        </a-col>
                        <a-col class="applyTrue " :span="4">
                          <div>
                        ...
                        <a-col class="applyTrue" :span="8">
                          ...
                          <!-- <a-form-item :wrapperCol="{ lg: { span: 16 }, sm: { span: 16 } }">
                            <a-input v-decorator="['shenheren']" disabled style="margin-left:10px;" />
                          </a-form-item> -->
                        </a-col>
                        <a-col class="applyTrue" :span="4">
                          <div>
                ...
                <a-form @submit="handleSubmit" :form="applyInfoFrom">
                  ...
                  >
                    <a-input
                      ...
                      v-decorator="['bohuiren', { rules: [{ required: true, message: '请输入' }] }]"
                      placeholder="审核人"
                      name="bohuiren"
                    />
                  </a-form-item>
  ...
  left: 84%; top: 0;"
          ...
          >
            ...
            <template slot="action" slot-scope="text, record">
              <a slot="action" v-permission="{active:'munsele'}" @click="fint(record)">查看&nbsp;&nbsp;</a>
            </template>
          </a-table>
...
const columns = [
  ...
  { title: '状态', width: 200, align: 'center', dataIndex: 'Fzt', key: 'Fzt' },
  { title: '养殖场名称', width: 220, align: 'center', dataIndex: 'FyzName', key: 'FyzName' },
  { title: '地址',
    width: 300,
    align: 'center',
    dataIndex: 'Fdz',
  ...
  { title: '开户行', width: 250, align: 'center', dataIndex: 'Fkhh', key: 'Fkhh' },
...
export default {
  ...
  // 模糊查询当前页
      ...
      Ffddbr: '',
      Fyzname: '',
      Fshdw: sessionStorage.getItem('FSenterpriseName'),
      shenheinfo: false
    }
  },
  created () {
  ...
  methods: {
    ...
    // 待审核申请查询
     checkapply () {
       this.$axios
        .get(
          http +
            '/After/areaReviewed?Fyzname=' + this.Fyzname + '&FSunitId=' + this.FSunitId + '&offset=' +
            this.offset +
            '&limit=' +
            this.limit +
            '&FScTime=' +
          ...
          if (res.data.Result) {
            res.data.Data.forEach((item, index) => {
              if (index === 0) {
                ...
                // this.Fsqrrq = item.FScTime.replace(/T/g, ' ')
                if (item.FSmemo1) {
                ...
                //  console.log(this.Fsqrrq.replace(/T/g, ' '))
              }
            })
          }
    ...
    // 驳回确认
     handleNO () {
      //  url: http + '/After/reject?FGlidV=' + this.FstidOne + '&FSuserName=' + this.shPeople + '&Fshdw=动监站&Fshrq=' + getTime() + '&Fshyj=驳回&Fbhly=' + this.bohuiRe + '&shzt=',
       this.$axios({
    ...
    handleSubmit (e) {
      e.preventDefault()
      this.applyInfoFrom.validateFields((err, values) => {
        if (!err) {
          this.shPeople = this.applyInfoFrom.getFieldValue('bohuiren')
          this.bohuiRe = this.applyInfoFrom.getFieldValue('bohuiRe')
          console.log('Received values of applyInfoFrom: ', values)
          this.handleNO()
        }
      })
    },
