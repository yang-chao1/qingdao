      ...
      <div class="div1">
        <div class="div3">
          ...
          <div style="margin-top:35px;">
            <!-- 处理-->
            <a-button type="primary" style="height:40px" icon="plus" @click="showModal()">处理</a-button
            >
            <a-modal
              :footer="null"
              :title="this.fromdata==='add'?'屠宰检疫申报处理':'屠宰检疫申报详情'"
              :visible="visible"
              ...
              <a-form @submit="handleSubmit" :form="form">
                ...
                <a-row>
                  ...
                  <a-col :span="12">
                    ...
                    >
                      <a-date-picker show-time :disabled="!this.f" v-decorator="['DateQF', { rules: [{ required: true, message: '请输入' }] }]" style="width: 100%" placeholder="请输入更新日期" />
                    </a-form-item>
                  </a-col>
                </a-row>
                <a-row>
                  ...
                  <a-col :span="12">
                    ...
                    >
                      <a-input
                        :maxLength="11"
                        :disabled="!this.f"
                        v-decorator="['QCPPhone', { rules: [{required: true, pattern: /^1[3456789]\d{9}$/, message: '请输入正确的手机号' }] }]"
                        name="QCPPhone"
                      />
                    </a-form-item>
                  </a-col>
                ...
                <a-row>
                  <a-col :span="12">
                    <a-form-item
                      ...
                      :wrapperCol="{ lg: { span: 12 }, sm: { span: 17 } }"
                    >
                      <a-date-picker show-time :disabled="!this.f" v-decorator="['DateQy', { rules: [{ required: true, message: '请输入' }] }]" style="width: 100%" placeholder="请输入更新日期" />
                    </a-form-item>
                  </a-col>
                  <a-col :span="12">
                    <a-form-item
                ...
                <a-row>
                  <a-col :span="12">
                    ...
                    >
                      <a-select
                        v-if="this.f"
                        v-decorator="['QCPAccepted', { rules: [{ required: true, message: '请选择' }] }]"
                      >
                        <a-select-option value="受理">受理</a-select-option>
                        <a-select-option value="不受理">不受理</a-select-option>
                      </a-select>
                      <a-input
                        v-if="!this.f"
                        disabled
                        v-decorator="['QCPAccepted', { rules: [{ required: true, message: '请输入' }] }]"
                        name="QCPAccepted"
                      />
                    </a-form-item>
                  </a-col>
                  <a-col :span="12">
                    ...
                    <a-date-picker :disabled="!this.f" v-decorator="['CLRQ', { rules: [{ required: true, message: '请输入' }] }]" style="width: 30%" placeholder="请输入更新日期" />
                    ...
                    <a-input
                      :disabled="!this.f"
                      style="width: 30%"
                      v-decorator="['QCPAddress', { rules: [{ required: true, message: '请输入' }] }]"
                      name="QCPAddress"
                    />
                ...
                <a-row>
                  <a-col :span="12">
                    ...
                    >
                      <a-input
                        :disabled="!this.f"
                        v-decorator="['QCPAttn', { rules: [{ required: true, message: '请输入' }] }]"
                        name="QCPAttn"
                      />
                  ...
                  <a-col :span="12">
                    ...
                    >
                      <a-input
                        :disabled="!this.f"
                        :maxLength="11"
                        v-decorator="['QCPJBRDianHua', { rules: [{ required: true,pattern: /^1[3456789]\d{9}$/, message: '请输入正确的手机号' }] }]"
                        name="QCPJBRDianHua"
                      />
                    </a-form-item>
                  ...
                  <a-col :span="12">
                    ...
                    >
                      <a-date-picker
                        ...
                        show-time
                        :disabled="!this.f"
                        v-decorator="['DateNpy', { rules: [{ required: true, message: '请输入' }] }]"
                        style="width: 100%"
                        placeholder="请输入更新日期" />
                    </a-form-item>
                  </a-col>
                  <span>(动物卫生监督机构 留存)</span>
                </a-row>
...
export default {
  ...
  methods: {
    ...
    showModal () {
      ...
      if (!this.delAllInfo.length || this.delAllInfo.length !== 1) {
        this.$notification.error({
          message: '请选择一条数据'
        })
      } else {
        if (this.delAllInfo[0].QCPAccepted !== '' && this.delAllInfo[0].QCPAccepted) {
          this.$notification.error({
            message: '该数据已被处理，请选择其他数据'
          })
        } else {
          this.f = true
          this.FStIditem = this.delAllInfo[0].FStId
    ...
    handleSubmit (e) {
      ...
      if (this.fromdata === 'update') {
        ...
        this.form.validateFields((err, val) => {
          if (!err) {
            var formInfo = this.form.getFieldsValue()
            ...
            this.itemInfo.QCPAccepted = formInfo.QCPAccepted
            this.itemInfo.CLRQ = formInfo.CLRQ
            this.itemInfo.QCPAddress = formInfo.QCPAddress
            this.itemInfo.QCPAttn = formInfo.QCPAttn
            this.itemInfo.QCPJBRDianHua = formInfo.QCPJBRDianHua
            this.itemInfo.DateNpy = formInfo.DateNpy
            this.itemInfo.IsPrant = '已保存'
            getQuarantineDeclarationCPUpdate(this.itemInfo).then(res => {
              console.log(res)
              if (res.Result) {
