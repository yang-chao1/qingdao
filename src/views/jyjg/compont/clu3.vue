<template>
  <div>
    ...
    <a-select v-model="dist" size="large" style="width: 120px" @change="handleChange">
      <a-select-option value="gmyzc">
        养殖场
      </a-select-option>

      <a-select-option value="syh">
        散养户
      </a-select-option>
      <a-select-option value="yzzyh">
        养殖专业户
      </a-select-option>
      <a-select-option value="zxqc">
        种畜禽场
      </a-select-option>
    </a-select>
    <a-input size="large" v-model="selectWhere" class="ainput" placeholder="请输入养殖场名称" />
...
export default {
  ...
  methods: {
    handleChange (val) {
     this.dist = val
        this.$emit('tiao', { offset: this.offset, limit: this.limit, selectWhere: this.selectWhere, dist: this.dist })
    },
    qu (data) {
      this.selectWhere = ''
      data.type = this.dist
    ...
    cx () {
 this.$emit('tiao', { offset: 1, limit: 10, selectWhere: this.selectWhere, dist: this.dist })
    },
     onChanges (current, pageSize) {
       this.offset = current
       this.limit = pageSize
       console.log(this.dist)
       this.$emit('tiao', { offset: current, limit: pageSize, selectWhere: this.selectWhere, dist: this.dist })
     },
