    ...
    <a-form @submit="handleSubmit" :form="form">
      <a-row>
        <a-col :span="12">
          ...
          >
            <a-input v-decorator="['FEPCName']" name="FEPCName" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item
      ...
      <a-row>
        <a-col :span="12" style="height: 120px;">
          <a-form-item
            ...
            :wrapperCol="{ lg: { span: 12 }, sm: { span: 17 } }"
          >
            <qr-code :visible="visibleUpdate" :codeId="this.$attrs.assistantItem.QRCode"></qr-code>
          </a-form-item>
        </a-col>
      </a-row>
...
import { getCFYYInforFileUp } from '@/api/assistant'
...
export default {
  ...
  watch: {
    assistantItem: function (newassistantItem, oldassistantItem) {
      console.log(newassistantItem, oldassistantItem)
      this.assistantItemInfo = newassistantItem
      // this.codeId = newassistantItem.QRCode
      this.FStId = newassistantItem.FStId
    }
  },
  created () {
    // 行政树
  ...
  methods: {
    fatherInfo () {
      this.FStId = this.$attrs.assistantItem.FStId
       this.$nextTick(() => {
        this.form.setFieldsValue(this.$attrs.assistantItem)
      })
    },
    selectOption (v) {
      console.log(v)
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          var p = {
            ...
            FEPCTime: this.assistantItemInfo.FEPCTime || this.$attrs.assistantItem.FEPCTime || '',
          ...
          getCFYYInforFileUp(p, this.UserId)
            .then(res => {
              console.log(res)
              this.ssyzTF = false
              this.reload()
